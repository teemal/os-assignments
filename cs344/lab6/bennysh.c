...
void free_list(cmd_list_t *cmd_list)
{
    ...
    cmd_t *temp_cmd1 = cmd_list->head;
    cmd_t *temp_cmd2 = temp_cmd1;
    param_t *temp_param1 = NULL;
    param_t *temp_param2 = NULL;

    while(temp_cmd1 != NULL) {
        temp_param1 = temp_cmd1->param_list;
        temp_param2 = temp_param1;
        free(temp_cmd2->raw_cmd);
        free(temp_cmd2->cmd);
        free(temp_cmd2->input_file_name);
        free(temp_cmd2->output_file_name);
        free(temp_cmd2->param_list);
        while(temp_param1 != NULL) {
                temp_param1 = temp_param1->next;
                free(temp_param2->param);
                temp_param2 = temp_param1;
        }
        temp_cmd1 = temp_cmd2->next;
        free(temp_cmd2);
        temp_cmd2 = temp_cmd1;
    }
}
