...
{   
    ...
    if (1 == cmds->count) {
        ...
        int p_trail = -1;
        int p[2] = {-1,-1};
        cmd_t *mult_cmd = cmds->head;
        while(mult_cmd){
            if(mult_cmd != cmds->tail){
                pipe(p);
                //check for errors
            }
            cpid = fork();
            switch(cpid){
                ...
                //child
                    {
                        ...
                        if(mult_cmd->input_src == REDIRECT_FILE){
                            int fd = open(mult_cmd->input_file_name, O_RDONLY);
                        ...
                        if(mult_cmd->output_dest == REDIRECT_FILE){
                            int fd = open(mult_cmd->output_file_name, O_WRONLY | O_CREAT | O_TRUNC | S_IRUSR | S_IWUSR);
                        ...
                        //build new argv and exec
                        nargs = calloc(mult_cmd->param_count + 2, sizeof(char *));
                        nargs[0] = mult_cmd->cmd;
                        for(i = 1, param = mult_cmd->param_list; param; param = param->next, i++){
                            nargs[i] = param->param;
                        }
                ...
                default:
                    ...
                    {
                        close(p[STDOUT_FILENO]);
                        p_trail = p[STDIN_FILENO];
                    }
                    break;
            } // closes switch
            mult_cmd = mult_cmd->next;
        } // closes while
        // {
        //     pid_t c_pid = -1;
        //     int status = -1;
        ...
        //         fprintf(stderr, "another one bites the dust....%u\n", (unsigned) c_pid);
        //     }
        // }
    } //  closes else
